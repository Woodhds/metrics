<template>
    <div class="bg-dark px-5 py-5 flex flex-col relative flex-wrap">
        <div class="flex flex-row mb-5 flex-wrap justify-between">
            ...
            <div class="sm:w-full rounded px-5 py-5 shadow-md md:w-1/3 flex flex-col" v-if="selectedMess.length > 0">
        ...
        <div v-if="messages.length > 0" class="flex flex-col">
            ...
            <ul class="flex list-reset mt-6" v-if="totalPages.length > 1">
                <li class="px-4 py-2 cursor-pointer" @click="page = item"
                    :class="[ item === page ? 'bg-blue text-white': '' ]" v-for="item of totalPages" :key="item">{{item}}
                </li>
            </ul>
        </div>
