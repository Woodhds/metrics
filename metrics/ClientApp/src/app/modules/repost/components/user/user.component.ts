...
export class UserComponent implements OnInit {
  ...
  like(owner_id: number, id: number) {
    let message = this.messages.find(a => a.Owner_Id === owner_id && a.Id === id);
    if (message && !message.Likes.User_Likes) {
      this.vkMessageService.like(owner_id, id)
        .subscribe(() => {
  ...
  repost(owner_id: number, id: number) {
    ...
    if (message && !message.Reposts.User_reposted) {
      this.vkMessageService.repost([new VkRepostModel(owner_id, id)])
        .subscribe(() => {
  ...
  repostAll() {
    this.vkMessageService.repost(
      this.selectedMessages.map(x => new VkRepostModel(x.Owner_Id, x.Id)), this.timeout)
      .subscribe(() => {
  ...
  onSelect(ev: MatSlideToggleChange, message: VkMessage) {
    message.IsSelected = ev.checked && !this.selectedMessages.some(f => f.Id === message.Id && f.Owner_Id === message.Owner_Id);
  }
