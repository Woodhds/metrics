...
namespace metrics.Services.Concrete
{
    public class VkMessageService : IVkMessageService
    {
        ...
        public async Task<DataSourceResponseModel> GetMessages(int page = 0, int take = 50, string search = null)
        {
            ...
            var response = await client.SearchAsync<VkMessage>(z =>
                z.From(page * take)
                    .Highlight(f => f.Fields(j => j.Field(y => y.Text).PreTags("em").PostTags("</em>")))
                    .Take(take)
                    .Query(f => f.MatchPhrase(q =>
                        q.Field(message => message.Text).Query(search))));
